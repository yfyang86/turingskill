...
description: |
  ...
  覆盖：1966-2025年A.M.图灵奖全部获奖者。
  用途：「用Knuth的视角分析」「哪个图灵奖得主适合思考这个问题」「蒸馏某位得主」。
...
| 系统构建 | 「如何设计可扩展的系统」「底层架构」 | Thompson/Ritchie（Unix）、Lampson（系统架构）、Thacker（Alto） |
| 编程语言 | 「语言设计」「类型系统」 | Perlis（ALGOL）、Wirth（Pascal）、Milner（ML）、Peyton Jones（Haskell） |
| 人工智能 | 「AI研究」「神经网络」 | Minsky（AI奠基）、Bengio/Hinton/LeCun（深度学习）、Barto/Sutton（强化学习）、Pearl（因果推理） |
| 理论计算 | 「可计算性」「复杂度」 | Cook（NP完全）、Karp（计算复杂性）、Wigderson（随机计算） |
| 数据库系统 | 「数据建模」「查询优化」 | Codd（关系模型）、Stonebraker（现代数据库）、Bachman（IDS） |
| 密码学安全 | 「加密」「安全协议」 | Diffie/Hellman（公钥密码）、Goldwasser/Micali（概率加密）、Rivest/Shamir/Adleman（RSA） |
| 软件工程 | 「程序验证」「形式化方法」 | Hoare（公理语义）、Dijkstra（正确性证明）、Pnueli（时序逻辑） |
| 图形学 | 「3D渲染」「计算机图形」 | Sutherland（Sketchpad）、Catmull/Hanrahan（Pixar渲染）、Torrance（光照模型） |
| 并行计算 | 「分布式系统」「并发」 | Valiant（并行计算）、Liskov（分布式系统）、Hillis（Connection Machine） |
| 长期主义 | 「坚持一个方向几十年」 | Knuth（TAOCP）、Sutherland（交互图形）、Iverson（APL） |
...
| 2023 | Avi Wigderson | 计算理论 | `avi-wigderson` |
...
| 2018 | Yann LeCun | 深度学习 | `yann-lecun` |
...
| 领域 | 得主 | 关键词 |
|------|------|--------|
| 🤖 人工智能 | Minsky, McCarthy, Newell/Simon, Feigenbaum/Reddy, Pearl, Bengio/Hinton/LeCun, Barto/Sutton | 神经网络、专家系统、因果推理、强化学习 |
| 💻 操作系统/系统 | Thompson/Ritchie, Lampson, Thacker, Engelbart, Corbato, Brooks | Unix、分布式、个人计算 |
| 🗄️ 数据库 | Codd, Bachman, Gray, Stonebraker | 关系模型、事务处理 |
| 🔒 密码学安全 | Rivest/Shamir/Adleman, Diffie/Hellman, Goldwasser/Micali, Bennett/Brassard | 公钥、量子密码 |
| 🌐 网络/互联网 | Cerf/Kahn, Metcalfe, Berners-Lee | TCP/IP、以太网、Web |
| 📝 编程语言 | Perlis, Backus, Wirth, Iverson, Milner, Dahl/Nygaard, Kay, Hoare | 编译器、类型系统、OO |
| 🧮 算法/理论 | Knuth, Cook, Karp, Rabin/Scott, Hartmanis/Stearns, Blum, Wigderson, Yao | 复杂性、图算法、计算理论 |
| 🎨 图形学 | Sutherland, Catmull/Hanrahan | CAD、渲染 |
| ⚡ 架构/硬件 | Hennessy/Patterson, Cocke, Wilkes | RISC、微程序 |
| 🔬 形式化方法 | Dijkstra, Hoare, Pnueli, Clarke/Emerson/Sifakis | 程序验证、模型检验 |
...
- [x] 覆盖1966-2025年全部图灵奖得主索引
- [ ] 蒸馏重点得主（Knuth, Thompson/Ritchie, Dijkstra等）
- [ ] 构建跨领域交叉索引
- [ ] 添加技术时间线对比
